...
convertArrayType(mlir::MLIRContext *context, arrow::ArrayType type,
  ...
  auto nullBitmapType =
      mlir::MemRefType::get(-1, mlir::IntegerType::get(context, 1));
  auto dataBufferType = mlir::MemRefType::get(-1, elementType);

  auto llvmDataBuffer = converter->convertType(dataBufferType);

  auto llvmNullBitmap = converter->convertType(nullBitmapType);

  auto indexType = mlir::IndexType::get(context);

  auto llvmLengthType = converter->convertType(indexType);
...
convertRecordBatchType(mlir::MLIRContext *context, arrow::RecordBatchType type,
                       mlir::TypeConverter *converter) {
  mlir::SmallVector<mlir::Type, 16> fieldTypes;
  auto indexType = converter->convertType(mlir::IndexType::get(context));

  auto rawPtrType = mlir::LLVM::LLVMPointerType::get(
      converter->convertType(mlir::IntegerType::get(context, 8)));
...
getOrInsertFuncSig(mlir::OpBuilder &builder, mlir::ModuleOp module,
                   mlir::StringRef name, mlir::Type resultType,
...
auto generateFunctionCall(mlir::OpBuilder &builder,
  ...
  mlir::SmallVector<mlir::Type, 6> argTypes;
  for (auto arg : args) {
    argTypes.push_back(typeConverter.convertType(arg.getType()));
  }
