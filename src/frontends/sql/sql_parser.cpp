...
void GroupByNode::visit(SqlVisitor &visitor) const { visitor.visit(*this); }
...
struct SqlGrammar
  ...
  qi::rule<Iter, std::shared_ptr<GroupByNode>(), qi::ascii::space_type>
      group_by;
  qi::rule<Iter, std::string(), qi::ascii::space_type> table_name;
  ...
  SqlGrammar() : SqlGrammar::base_type(start) {
    using qi::_1;
    using qi::_val;
    ...
    // clang-format off
    start = 
        "select" >> qi::eps [_val = make_shared_<SelectNode>()()]
        >> projection [bind(&SelectNode::projection, _val) =  _1] 
        >> "from"
        >> table_name [bind(&SelectNode::table_name, _val) = _1] 
        >> -filter [bind(&SelectNode::filter, _val) = _1] 
        >> -group_by [bind(&SelectNode::groub_by, _val) = _1] 
        >> ';';
...
const std::shared_ptr<SqlNode> parse_sql(std::string sql) {
  SqlGrammar<std::string::iterator> grammar;
