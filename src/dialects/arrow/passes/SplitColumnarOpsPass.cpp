...
public:
  ...
  matchAndRewrite(Op op, mlir::PatternRewriter &rewriter) const override {
    ...
    auto resultType = rewriter.getType<ColumnType>(
        rewriter.getI1Type(), columnType.getChunksCount(),
        columnType.getChunkLengths());
    ...
    for (int i = 0; i < columnType.getChunksCount(); i++) {
      ...
      if (rhs.getType().isa<ColumnType>()) {
        rhs = rewriter.create<GetChunkOp>(loc, chunkType, rhs, i);
      }
      rewriter.create<SetChunkOp>(
          loc, res, i, rewriter.create<Op>(loc, chunkType, lhs, rhs));
    }
    ...
    func.walk([&](mlir::Operation *op) {
      ...
      if (auto getColumnOp = mlir::dyn_cast<GetColumnOp>(op)) {
        ...
        for (int i = 0; i < columnType.getChunksCount(); i++) {
          ...
          chunks.push_back(
              builder.create<GetArrayOp>(op->getLoc(), arrayType, colIdx, i));
        }
